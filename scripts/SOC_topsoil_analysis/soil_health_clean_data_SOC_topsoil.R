...
## read in data ####
soil_health_raw_data_topsoil_SOC <- read.csv("~/Desktop/research/UMD_org_soil_MA/Crystal-Ornelas_Thapa_Tully_datasheet - datasheet_topsoil_SOC_v2.csv", header = TRUE)
...
raw_data_SOC_with_nas_topsoil <- dplyr::select(soil_health_raw_data_topsoil_SOC, 
                                               study_code,
                                               first_author,
                                               control_mean_standardized_SOC, 
                                               control_sd_standardized_SOC, 
                                               control_n,
                                               treatment_mean_standardized_SOC,
                                               treatment_sd_standardized_SOC,
                                               treatment_n)

head(raw_data_SOC_with_nas_topsoil)

# Imputation for SOC data ####
raw_data_SOC_with_nas_topsoil <- raw_data_SOC_with_nas_topsoil %>%
  mutate(
    study_code = as.factor(study_code),
    control_mean_standardized_SOC = as.numeric(control_mean_standardized_SOC),
    control_sd_standardized_SOC = as.numeric(control_sd_standardized_SOC),
    control_n = as.numeric(control_n),
    treatment_mean_standardized_SOC = as.numeric(treatment_mean_standardized_SOC),
    treatment_sd_standardized_SOC = as.numeric(treatment_sd_standardized_SOC),
...
meth[c("control_mean_standardized_SOC","treatment_mean_standardized_SOC","control_n","treatment_n")]=""

# What are we going to impute, gotta pick different methods for ordinal, factors or continuous
meth[c("control_sd_standardized_SOC","treatment_sd_standardized_SOC")]="norm" 
...
soil_health_raw_data_topsoil_SOC$control_sd_standardized_SOC <- imputed_sds_SOC_topsoil$control_sd_standardized_SOC
soil_health_raw_data_topsoil_SOC$treatment_sd_standardized_SOC <- imputed_sds_SOC_topsoil$treatment_sd_standardized_SOC
